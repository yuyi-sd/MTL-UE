...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_representation = False, mask = None):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        ...
        if mask is not None:
            feature = feature * mask + feature.detach() * (1 - mask)
...
class ResNetAE(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, stem_end_block = 13, latent_dim = 128):
        ...
        self.multiplier = latent_dim / 128
        self.in_planes = max(int(64 * self.multiplier), 1)
        self.conv1 = nn.Conv2d(3, max(int(64 * self.multiplier), 1), kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(max(int(64 * self.multiplier), 1))
        self.layer1 = self._make_layer(block, max(int(64 * self.multiplier), 1), num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, latent_dim, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        self.linear = nn.Linear(512 * block.expansion, num_classes)
        self.pooling = nn.AdaptiveAvgPool2d((1, 1))
        ...
        if self.stem_end_block == 5:
            ...
            self.decoder1 = nn.Sequential(
                nn.ConvTranspose2d(latent_dim, max(int(64 * self.multiplier), 1), 4, stride=2, padding=1),  
                nn.ConvTranspose2d(max(int(64 * self.multiplier), 1), max(int(64 * self.multiplier), 1), 3, stride=1, padding=1), 
                nn.InstanceNorm2d(max(int(64 * self.multiplier), 1)), 
                nn.ReLU(),
            )
            self.decoder2 = nn.Sequential(
                nn.ConvTranspose2d(max(int(64 * self.multiplier), 1), max(int(32 * self.multiplier), 1), 3, stride=1, padding=1),  
                nn.InstanceNorm2d(max(int(32 * self.multiplier), 1)), 
                nn.ReLU(),
            )
            self.decoder3 = nn.Sequential(
                nn.ConvTranspose2d(max(int(32 * self.multiplier), 1), 3, 3, stride=1, padding=1), 
                nn.Sigmoid(),
            )
            self.decoder = nn.Sequential(
                self.decoder1, self.decoder2, self.decoder3
...
def ResNet18AE(num_classes=10, stem_end_block=15, latent_dim = 128):
    return ResNetAE(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, stem_end_block=stem_end_block, latent_dim = latent_dim)
...
class ResNetDVAE(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, stem_end_block = 9, latent_dim = 128, imagenet = False):
        ...
        if self.stem_end_block == 5:
            ...
            if self.spatial_emb:
                self.emb_p = nn.Embedding(num_classes, 256 * 56 * 56)
            else:
                self.emb_p = nn.Embedding(num_classes, 256)
            self.decoder_p1 = nn.Sequential(
                nn.ConvTranspose2d(256, 128, 4, stride=2, padding=1),  
                nn.ConvTranspose2d(128, 128, 3, stride=1, padding=1), 
                nn.InstanceNorm2d(128), 
            ...
            if self.spatial_emb:
                self.emb_p = nn.Embedding(num_classes, 512 * 28 * 28)
            else:
                self.emb_p = nn.Embedding(num_classes, 512)
            self.decoder_p1 = nn.Sequential(
                nn.ConvTranspose2d(512, 256, 4, stride=2, padding=1), 
                nn.ConvTranspose2d(256, 128, 3, stride=1, padding=1),
                nn.InstanceNorm2d(256), 
    ...
    def set_spatial_emb(self, dim = 128, h = 16):
        if self.spatial_emb:
            self.emb_p = nn.Embedding(self.num_classes, dim * h * h)
        else:
            self.emb_p = nn.Embedding(self.num_classes, dim)
...
def ResNet18DVAE(num_classes=10, stem_end_block=9, latent_dim = 128, imagenet = False):
    return ResNetDVAE(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, stem_end_block=stem_end_block, latent_dim = latent_dim, imagenet = imagenet)
...
class ResNetDVAE_pure(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, stem_end_block = 13, latent_dim = 128, imagenet = False):
        super(ResNetDVAE_pure, self).__init__()
        self.in_planes = 64
        self.imagenet = imagenet
        if self.imagenet:
            self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def forward(self, x, y):
        ...
        if self.sample_wise:
            ...
            perturbation = self.decoder_p(Embedding)
        return rec, mu, log_var, perturbation
...
def ResNet18DVAE_pure(num_classes=10, stem_end_block=15, latent_dim = 128, imagenet = False):
    return ResNetDVAE_pure(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, stem_end_block=stem_end_block, latent_dim = latent_dim, imagenet = imagenet)



class ResNetDAE(nn.Module):
    def __init__(self, block, num_blocks, num_classes=2, stem_end_block = 9, attr_num = 40, imagenet = False, epsilon = 0.0314, emb_p_spatial_dim = 35, in_channels=3, sample_wise = True, z_only = False):
        super(ResNetDAE, self).__init__()
        ...
        if not isinstance(self.num_classes, list):
            self.num_classes = [self.num_classes] * self.attr_num
        self.linear = nn.Linear(512 * block.expansion, self.num_classes[0])

        print (self.num_classes)
        self.stem_end_block = stem_end_block
        self.emb_p_dim = 16
        self.emb_p_spatial_dim = emb_p_spatial_dim
        print (self.stem_end_block)
        if self.stem_end_block in [7,9]:
            if self.spatial_emb:
                for i in range(self.attr_num):
                    setattr(self, f"emb_p_{i}", nn.Embedding(self.num_classes[i], self.emb_p_dim * self.emb_p_spatial_dim * self.emb_p_spatial_dim))
            else:
                for i in range(self.attr_num):
                    setattr(self, f"emb_p_{i}", nn.Embedding(self.num_classes[i], self.emb_p_dim))
            if not self.imagenet:
                if self.sample_wise:
                    if self.z_only:
                        input_dim = 128
                    else:
                        input_dim = 128 + self.emb_p_dim * self.attr_num
                else:
                    input_dim = self.emb_p_dim * self.attr_num
                print (input_dim)
                self.decoder_p1 = nn.Sequential(
                    nn.ConvTranspose2d(input_dim, 64, 4, stride=2, padding=1),  
                    nn.ConvTranspose2d(64, 64, 3, stride=1, padding=1), 
                    nn.InstanceNorm2d(64), 
                    nn.ReLU(),
                )
                ...
                self.decoder_p3 = nn.Sequential(
                    nn.ConvTranspose2d(32, self.in_channels, 3, stride=1, padding=1), 
                    # nn.Sigmoid(),
                )
                self.decoder_p = nn.Sequential(
                    self.decoder_p1, self.decoder_p2, self.decoder_p3
            ...
            if self.spatial_emb:
                for i in range(self.attr_num):
                    setattr(self, f"emb_p_{i}", nn.Embedding(num_classes[i], 256 * 56 * 56))
            else:
                for i in range(self.attr_num):
                    setattr(self, f"emb_p_{i}", nn.Embedding(num_classes[i], 256))
            self.decoder_p1 = nn.Sequential(
                nn.ConvTranspose2d(256, 128, 4, stride=2, padding=1),  
                nn.ConvTranspose2d(128, 128, 3, stride=1, padding=1), 
                nn.InstanceNorm2d(128), 
            ...
            self.decoder_p3 = nn.Sequential(
                nn.ConvTranspose2d(64, self.in_channels, 3, stride=1, padding=1), 
                # nn.Sigmoid(),
            )
            self.decoder_p = nn.Sequential(
                self.decoder_p1, self.decoder_p2, self.decoder_p3
            ...
            if self.spatial_emb:
                for i in range(self.attr_num):
                    setattr(self, f"emb_p_{i}", nn.Embedding(num_classes[i], 512 * 28 * 28))
            else:
                for i in range(self.attr_num):
                    setattr(self, f"emb_p_{i}", nn.Embedding(num_classes[i], 512))
            self.decoder_p1 = nn.Sequential(
                nn.ConvTranspose2d(512, 256, 4, stride=2, padding=1), 
                nn.ConvTranspose2d(256, 128, 3, stride=1, padding=1),
                nn.InstanceNorm2d(256), 
    ...
    def forward(self, x, y, binary = True, partial_protect = False, no_protect_first=10, no_protect_index = [0]):
        z = self.forward_stem(x)
        if self.first:
            if binary:
                if self.spatial_emb:
                    if partial_protect:
                        no_protect_Embedding = torch.cat([((getattr(self, f"emb_p_{i}")(torch.zeros_like(y[:, i]))+getattr(self, f"emb_p_{i}")(torch.ones_like(y[:, i])))/2).reshape(z.shape[0], self.emb_p_dim, z.shape[2], z.shape[3]) for i in range(no_protect_first)], dim=1)
                        protect_Embedding = torch.cat([getattr(self, f"emb_p_{i}")(y[:, i]).reshape(z.shape[0], self.emb_p_dim, z.shape[2], z.shape[3]) for i in range(no_protect_first, self.attr_num)], dim=1)
                        Embedding = torch.cat([no_protect_Embedding, protect_Embedding], dim = 1)
                    else:
                        Embedding = torch.cat([getattr(self, f"emb_p_{i}")(y[:, i]).reshape(z.shape[0], self.emb_p_dim, z.shape[2], z.shape[3]) for i in range(self.attr_num)], dim=1)
                else:
                    Embedding = torch.cat([getattr(self, f"emb_p_{i}")(y[:, i])[:,:,None,None].repeat(1, 1, z.shape[2], z.shape[3]) for i in range(self.attr_num)], dim=1)
            else:
                if self.spatial_emb:
                    if partial_protect:
                        if no_protect_first:
                            no_protect_Embedding = torch.tensor([]).cuda()
                            for i in range(no_protect_first):
                                embeddings = torch.stack([getattr(self, f"emb_p_{i}")(j * torch.ones_like(y[i]).int()).reshape(z.shape[0], self.emb_p_dim, z.shape[2], z.shape[3]) for j in range(self.num_classes[i])], dim=0).mean(0)
                                no_protect_Embedding = torch.cat([no_protect_Embedding, embeddings], dim = 1)
                            protect_Embedding = torch.cat([getattr(self, f"emb_p_{i}")(y[i].int()).reshape(z.shape[0], self.emb_p_dim, z.shape[2], z.shape[3]) for i in range(no_protect_first, self.attr_num)], dim=1)
                            Embedding = torch.cat([no_protect_Embedding, protect_Embedding], dim = 1)
                        else:
                            Embedding = torch.tensor([]).cuda()
                            for i in range(self.attr_num):
                                if i in no_protect_index:
                                    embeddings = torch.stack([getattr(self, f"emb_p_{i}")(j * torch.ones_like(y[i]).int()).reshape(z.shape[0], self.emb_p_dim, z.shape[2], z.shape[3]) for j in range(self.num_classes[i])], dim=0).mean(0)
                                else:
                                    embeddings = getattr(self, f"emb_p_{i}")(y[i].int()).reshape(z.shape[0], self.emb_p_dim, z.shape[2], z.shape[3])
                                Embedding = torch.cat([Embedding, embeddings], dim=1)
                    else:
                        Embedding = torch.cat([getattr(self, f"emb_p_{i}")(y[i].int()).reshape(z.shape[0], self.emb_p_dim, z.shape[2], z.shape[3]) for i in range(self.attr_num)], dim=1)
                else:
                    Embedding = torch.cat([getattr(self, f"emb_p_{i}")(y[i].int())[:,:,None,None].repeat(1, 1, z.shape[2], z.shape[3]) for i in range(self.attr_num)], dim=1)
        else:
            raise('Not implemented')
        if self.sample_wise:
            if self.z_only:
                perturbation = self.decoder_p(z)
            else:
                perturbation = self.decoder_p(torch.cat([Embedding, z], dim = 1))
        else:
            perturbation = self.decoder_p(Embedding)
        perturbation = perturbation.clamp(-self.epsilon, self.epsilon)
        return perturbation

    def embedding_cosine_similarity_loss(self):
        loss = 0
        for i in range(self.attr_num):
            embedding = getattr(self, f"emb_p_{i}")
            embeddings = embedding(torch.arange(self.num_classes[i]).int().cuda())
            cosine_sim_matrix = F.cosine_similarity(embeddings.unsqueeze(1), embeddings.unsqueeze(0), dim=2)
            mask = ~torch.eye(cosine_sim_matrix.size(0), dtype=bool).cuda()
            non_diag_elements = cosine_sim_matrix[mask]
            loss = loss + (non_diag_elements.mean() + 1)
        loss = loss / self.attr_num
        return loss

    def cross_embedding_cosine_similarity_loss(self):
        if self.attr_num == 1:
            return 0
        cosine_similarities = []
        for i in range(self.attr_num):
            for j in range(i + 1, self.attr_num):
                embedding_i = getattr(self, f"emb_p_{i}").weight
                embedding_j = getattr(self, f"emb_p_{j}").weight
                cosine_similarity_matrix = F.cosine_similarity(embedding_i.unsqueeze(1), embedding_j.unsqueeze(0), dim=-1)
                cosine_similarities.append(cosine_similarity_matrix)
        all_cosine_similarities = torch.cat([sim.flatten() for sim in cosine_similarities])
        loss = all_cosine_similarities.abs().mean()
        return loss

def ResNet18DAE(num_classes=2, stem_end_block=9, attr_num = 40, imagenet = False, epsilon = 0.0314, emb_p_spatial_dim = 35, in_channels=3, sample_wise = True, z_only = False):
    return ResNetDAE(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, stem_end_block=stem_end_block, attr_num = attr_num, imagenet = imagenet, epsilon = epsilon, emb_p_spatial_dim = emb_p_spatial_dim, in_channels=in_channels, sample_wise = sample_wise, z_only = z_only)



class AlphaConsistencyModule(nn.Module):
    def __init__(self, latent_shape, num_basis=10):
        super().__init__()
        self.num_basis = num_basis
        D, H, W = latent_shape

        # Register basis as non-trainable buffer
        basis = self.generate_orthogonal_basis(num_basis, D, H, W)
        self.register_buffer("basis", basis)

        # α from x or residual
        self.img_to_alpha_logits = nn.Sequential(
        ...
        # α from predicted perturbation
        self.p_to_alpha_logits = nn.Sequential(
            nn.Conv2d(3, 128, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
    ...
    def forward(self, x, z):
        alpha_logits = self.img_to_alpha_logits(x)      # [B, K]
        alpha = F.softmax(alpha_logits, dim=-1)

        # Apply basis combination
        B = x.size(0)
        basis_exp = self.basis.unsqueeze(0).expand(B, -1, -1, -1, -1)  # [B, K, D, H, W]
        weighted = torch.sum(alpha[:, :, None, None, None] * basis_exp, dim=1)  # [B, D, H, W]

        return z + weighted, alpha

    @staticmethod
    def generate_orthogonal_basis(K, D, H, W):
        A = torch.randn(D * H * W, K)
        Q, _ = torch.linalg.qr(A)  # [DHW, K]
        basis = Q.T.reshape(K, D, H, W)
        basis = basis / basis.view(K, -1).norm(dim=1, keepdim=True).view(K, 1, 1, 1)
        return basis
...
class ResNetDVAElf(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, stem_end_block = 9, latent_dim = 128, latent_shape = 16, num_basis=100, imagenet = False):
        super(ResNetDVAElf, self).__init__()
        self.in_planes = 64
        self.imagenet = imagenet
        if self.imagenet:
            self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        self.spatial_basis_module = AlphaConsistencyModule(
            latent_shape=(128, latent_shape, latent_shape),
            num_basis=num_basis
        )
    ...
    def forward(self, x):
        ...
        # x_feat = x  
        x_feat = x - rec
        z_plus, alpha = self.spatial_basis_module(x_feat, z)
        perturbation = self.decoder_p(z_plus)
        
        # alpha_p_logits = self.spatial_basis_module.p_to_alpha_logits(perturbation)  # [B, K]
        alpha_p_logits = self.spatial_basis_module.img_to_alpha_logits(perturbation)  # [B, K]
        alpha_p = F.softmax(alpha_p_logits, dim=-1)

        return logits, rec, mu, log_var, perturbation, alpha, alpha_p
...
def ResNet18DVAElf(num_classes=10, stem_end_block=9, latent_dim = 128, latent_shape = 16, num_basis=100, imagenet = False):
    return ResNetDVAElf(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, stem_end_block=stem_end_block, latent_dim = latent_dim, latent_shape = latent_shape, num_basis=num_basis, imagenet = imagenet)

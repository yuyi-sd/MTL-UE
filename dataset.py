...
transform_options = {
    ...
    'WebFace': {
        ...
        "test_transform": [transforms.ToTensor()]},
    'FairFace': {
        "train_transform": [transforms.RandomCrop(128, padding=0),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor()],
        "test_transform": [transforms.CenterCrop(128),
            transforms.ToTensor()]},
    'PoisonFairFace': {
       "train_transform": [transforms.RandomCrop(128, padding=0),
            transforms.RandomHorizontalFlip(),
    ...
    'PoisonUTKFace': {
        ...
        "test_transform": [transforms.CenterCrop(128),
            transforms.ToTensor()]},
    'CelebA': {
        "train_transform": [transforms.RandomCrop(64, padding=0),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor()],
        "test_transform": [transforms.CenterCrop(64),
            transforms.ToTensor()]},
    'PoisonCelebA': {
        "train_transform": [transforms.RandomCrop(64, padding=0),
            transforms.RandomHorizontalFlip(),
        ...
        "test_transform": [transforms.CenterCrop(64),
            transforms.ToTensor()]},
    'ChestXray14': {
        "train_transform": [transforms.RandomCrop(112, padding=0),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor()],
        "test_transform": [transforms.CenterCrop(112),
            transforms.ToTensor()]},
    'PoisonChestXray14': {
        "train_transform": [transforms.RandomCrop(112, padding=0),
            transforms.RandomHorizontalFlip(),
        ...
        "test_transform": [transforms.CenterCrop(112),
            transforms.ToTensor()]},
    'ChestXray14_256': {
        "train_transform": [transforms.RandomCrop(224, padding=0),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor()],
        "test_transform": [transforms.CenterCrop(224),
            transforms.ToTensor()]},
    'PoisonChestXray14_256': {
        "train_transform": [transforms.RandomCrop(224, padding=0),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor()],
        "test_transform": [transforms.CenterCrop(224),
...
transform_options['PoisonCIFAR10_CMTL'] = transform_options['CIFAR10']
...
transform_options['CIFAR10_Tensor'] = transform_options['CIFAR10']
...
class DatasetGenerator():
    def __init__(self, train_batch_size=128, eval_batch_size=256, num_of_workers=4,
                 ...
                 use_cutout=None, use_cutmix=False, use_mixup=False, identity = 'race', load_to_memory = False, grayscale = False, jpeg = False, bdr = False, poison_samples_idx = None, attribute_number = 0, partial_rate = 1.0):
        ...
        if train_data_type == 'CIFAR10':
        ...
        elif train_data_type == 'CIFAR10_Tensor':
            num_of_classes = 10
            train_dataset = CIFAR10_Tensor(root=train_data_path, train=True,
                                             download=True, transform=train_transform)
        elif train_data_type == 'CIFAR10_CMTL':
            num_of_classes = 10
            train_dataset = CIFAR10_CMTL(root=train_data_path, train=True,
                                             download=True, transform=train_transform)
        ...
        elif train_data_type == 'PoisonCIFAR10_CMTL':
            num_of_classes = 10
            train_dataset = PoisonCIFAR10_CMTL(root=train_data_path, transform=train_transform,
                                          poison_rate=poison_rate, perturb_type=perturb_type,
                                          patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                          perturb_tensor_filepath=perturb_tensor_filepath,
                                          add_uniform_noise=add_uniform_noise,
            ...
            #                                target_type="identity", transform=test_transform)
            train_dataset = CelebADataset(root=train_data_path, split="train", transform=train_transform, partial_rate=partial_rate)
            test_dataset = CelebADataset(root=test_data_path, split="test", transform=test_transform)
        elif train_data_type == 'PoisonCelebA':
            # train_dataset = datasets.CelebA(root=train_data_path, split="all",
            #                                 target_type="identity", transform=train_transform)
            # test_dataset = datasets.CelebA(root=train_data_path, split="all",
            #                                target_type="identity", transform=test_transform)
            train_dataset = PoisonCelebADataset(root=train_data_path, split="train", transform=train_transform,
                                           poison_rate=poison_rate, perturb_type=perturb_type,
                                           patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                           perturb_tensor_filepath=perturb_tensor_filepath,
                                           add_uniform_noise=add_uniform_noise,
                                           poison_classwise=poison_classwise,
                                           poison_classwise_idx=poison_classwise_idx, attribute_number = attribute_number, partial_rate=partial_rate)
            test_dataset = CelebADataset(root=test_data_path, split="test", transform=test_transform)
        elif train_data_type == 'FairFace':
            train_dataset = FairFaceDataset(root=train_data_path, split = 'train', transform=train_transform)
        elif train_data_type == 'PoisonFairFace':
            train_dataset = PoisonFairFaceDataset(root=train_data_path, split = 'train', transform=train_transform,
                                           poison_rate=poison_rate, perturb_type=perturb_type,
                                           patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                           perturb_tensor_filepath=perturb_tensor_filepath,
                                           add_uniform_noise=add_uniform_noise,
                                           poison_classwise=poison_classwise,
                                           poison_classwise_idx=poison_classwise_idx)
        elif train_data_type == 'UTKFace':
            train_dataset = UTKFaceDataset(root=train_data_path, split = 'train', transform=train_transform, partial_rate=partial_rate)
        elif train_data_type == 'PoisonUTKFace':
            train_dataset = PoisonUTKFaceDataset(root=train_data_path, split = 'train', transform=train_transform,
                                           poison_rate=poison_rate, perturb_type=perturb_type,
                                           patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                           perturb_tensor_filepath=perturb_tensor_filepath,
                                           add_uniform_noise=add_uniform_noise,
                                           poison_classwise=poison_classwise,
                                           poison_classwise_idx=poison_classwise_idx, partial_rate=partial_rate)
        elif train_data_type == 'ChestXray14':
            train_dataset = ChestXray14(root=train_data_path, split = 'train', transform=train_transform)
        elif train_data_type == 'PoisonChestXray14':
            train_dataset = PoisonChestXray14(root=train_data_path, split = 'train', transform=train_transform,
                                           poison_rate=poison_rate, perturb_type=perturb_type,
                                           patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                           perturb_tensor_filepath=perturb_tensor_filepath,
                                           add_uniform_noise=add_uniform_noise,
                                           poison_classwise=poison_classwise,
                                           poison_classwise_idx=poison_classwise_idx)
        elif train_data_type == 'ChestXray14_256':
            train_dataset = ChestXray14(root=train_data_path, split = 'train', transform=train_transform, resize = 256)
        elif train_data_type == 'PoisonChestXray14_256':
            train_dataset = PoisonChestXray14(root=train_data_path, split = 'train', transform=train_transform, resize = 256, 
                                           poison_rate=poison_rate, perturb_type=perturb_type,
                                           patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                           perturb_tensor_filepath=perturb_tensor_filepath,
                                           add_uniform_noise=add_uniform_noise,
        ...
        if test_data_type == 'CIFAR10':
        ...
        elif test_data_type == 'CIFAR10_Tensor':
            test_dataset = CIFAR10_Tensor(root=test_data_path, train=False,
                                            download=True, transform=test_transform)
        elif test_data_type == 'CIFAR10_CMTL':
            test_dataset = CIFAR10_CMTL(root=test_data_path, train=False,
                                            download=True, transform=test_transform)
            ...
            test_dataset = FairFaceDataset(root=test_data_path, split = 'test', transform=test_transform)
        elif test_data_type == 'UTKFace':
            test_dataset = UTKFaceDataset(root=test_data_path, split = 'test', transform=test_transform)
        elif test_data_type == 'ChestXray14':
            test_dataset = ChestXray14(root=test_data_path, split = 'test', transform=test_transform)
        elif test_data_type == 'ChestXray14_256':
            test_dataset = ChestXray14(root=test_data_path, split = 'test', transform=test_transform, resize=256)
        elif test_data_type == 'CIFAR10C':
            test_dataset = CIFAR10CDataset(root=test_data_path, split = 'test', transform=test_transform)
...
class ChestXray14(Dataset):
    def __init__(self, root, split="train", transform=None,
                 target_transform=None, download=False, resize = 128):
        super(ChestXray14, self).__init__()
        self.root = root
        self.transform = transform
        self.split = split
        self.target_transform = target_transform
        data = torch.load(os.path.join(self.root, split + '_data_{}.pt'.format(resize)))
        self.image = []
        for image_data in data['images']:
          self.image.append(Image.fromarray(image_data.numpy()))
        self.attr = data['labels'].numpy()
        del data
        print (len(self.image))
        self.w_transform = True
    ...
    def __getitem__(self, index):
        ...
        if (self.transform is not None) and self.w_transform:
            X = self.transform(X)
        else:
        ...
        if not self.return_index:
            return X, torch.tensor(target).float()
        else:
            return X, torch.tensor(target).float(), torch.tensor(index)

class PoisonChestXray14(Dataset):
    def __init__(self, root, split="train", transform=None,
                 target_transform=None, download=False, resize = 128, 
                 poison_rate=1.0, perturb_tensor_filepath=None,
                 seed=0, perturb_type='samplewise', patch_location='center', img_denoise=False,
                 add_uniform_noise=False, poison_classwise=False, poison_classwise_idx=None, attribute_number = 0):
        super(PoisonChestXray14, self).__init__()
        self.perturb_tensor = torch.load(perturb_tensor_filepath, map_location=torch.device('cpu'))
        print(self.perturb_tensor.shape)
        if len(self.perturb_tensor.shape) == 4:
        ...
        for idx in self.poison_samples_idx:
            ...
            if perturb_type == 'samplewise':
                # Sample Wise poison
                noise = perturb_tensor[idx]
                noise = patch_noise_extend_to_img(noise, [noise_length, noise_length, 1], patch_location=self.patch_location)
            elif perturb_type == 'classwise':
                # Class Wise Poison
                noise = perturb_tensor[self.attr[idx, self.attribute_number]]
                noise = patch_noise_extend_to_img(noise, [noise_length, noise_length, 1], patch_location=self.patch_location)
            elif 'classwise_spatial' in perturb_type:
              # Class Wise Poison
                num_attr = int(perturb_type.split('_')[-1])
                binary_list = self.attr[idx, :num_attr]
                noise = torch.zeros(perturb_tensor.shape[1], perturb_tensor.shape[2], perturb_tensor.shape[3])
                for i, val in enumerate(binary_list):
                    if val == 0:
                        noise += perturb_tensor[2 * i, :, :, :]
                    else:
                        noise += perturb_tensor[2 * i + 1, :, :, :]
                noise = noise / num_attr
                noise = patch_noise_extend_to_img(noise, [noise_length, noise_length, 1], patch_location=self.patch_location)
            elif 'classwise_patch' in perturb_type:
              # Class Wise Poison
                import math
                num_attr = int(perturb_type.split('_')[-1])
                N = 2**(math.ceil(math.log2(math.ceil(num_attr ** 0.5))))
                patch_size = (noise_length * (224 / 256)) // N
                patch_size = int(patch_size)
                binary_list = self.attr[idx, :num_attr]
                noise = torch.zeros(perturb_tensor.shape[1], perturb_tensor.shape[2], perturb_tensor.shape[3])
                for i in range(N):
                    for j in range(N):
                        iidx = i * N + j  # 计算第 iidx 个 patch 的索引
                        if iidx >= num_attr:  # 如果超出 binary_list 的长度，跳过
                            break
                        if binary_list[iidx] == 0:
                            patch_perturbation = perturb_tensor[2 * iidx, ::N, ::N, :]
                        else:
                            patch_perturbation = perturb_tensor[2 * iidx + 1, ::N, ::N, :]
                        noise[i * patch_size:(i + 1) * patch_size, j * patch_size:(j + 1) * patch_size, :] += patch_perturbation
                noise = patch_noise_extend_to_img(noise, [noise_length, noise_length, 1], patch_location=self.patch_location)
            if add_uniform_noise:
                noise += np.random.uniform(0, 8, (noise_length, noise_length, 1))
            self.image[idx] = np.array(self.image[idx]).astype(np.float32)
            self.image[idx] = self.image[idx] + np.squeeze(noise)
            self.image[idx] = np.clip(self.image[idx], a_min=0, a_max=255)
            self.image[idx] = self.image[idx].astype(np.uint8)
            self.image[idx] = Image.fromarray(self.image[idx]).convert('L')
        print('add_uniform_noise: ', add_uniform_noise)
        print(self.perturb_tensor.shape)
        print('Poison samples: %d/%d' % (len(self.poison_samples), len(self)))
    def __len__(self):
...
class CelebADataset(datasets.CelebA):
    def __init__(self, root, split="train", target_type="attr", transform=None,
                 target_transform=None, download=False, partial_rate=1.0):
        super(CelebADataset, self).__init__(root=root, split=split, target_type=target_type,
                                         transform=transform, target_transform=target_transform,
                                         download=False)
        # self.filename = self.filename[:10000]
        print (len(self.filename))
        if os.path.exists(os.path.join(self.root, self.base_folder, split + '_images_dict.pkl')):
            with open(os.path.join(self.root, self.base_folder, split + '_images_dict.pkl'), 'rb') as f:
                self.image = pickle.load(f)
        else:
            self.image = {}
            for index in range(len(self.filename)):
                filename = self.filename[index]
                image = Image.open(os.path.join(self.root, self.base_folder, "img_align_celeba", filename))
                image = transforms.CenterCrop((140, 140))(image)
                image = transforms.Resize((70, 70))(image)
    ...
    def __len__(self):
        return int(len(self.filename) * self.partial_rate)
    ...
    def __getitem__(self, index):
        ...
        if not self.return_index:
            ...
            return X, target.float(), torch.tensor(index)


class PoisonCelebADataset(datasets.CelebA):
    def __init__(self, root, split="train", target_type="attr", transform=None,
                 target_transform=None, download=False,
                 poison_rate=1.0, perturb_tensor_filepath=None,
                 seed=0, perturb_type='samplewise', patch_location='center', img_denoise=False,
                 add_uniform_noise=False, poison_classwise=False, poison_classwise_idx=None, attribute_number = 0, partial_rate=1.0):
        super(PoisonCelebADataset, self).__init__(root=root, split=split, target_type=target_type,
                                         transform=transform, target_transform=target_transform,
                                         download=False)
        print (len(self.filename))
        self.partial_rate = partial_rate
        ...
        # Check Shape
        target_dim = self.perturb_tensor.shape[0] if len(self.perturb_tensor.shape) == 4 else self.perturb_tensor.shape[1]
        if perturb_type == 'samplewise' and target_dim != len(self.filename):
            raise('Poison Perturb Tensor size not match for samplewise')
        # elif perturb_type == 'classwise' and (not (target_dim in [9,2,5])):
        #     raise('Poison Perturb Tensor size not match for classwise')
        ...
        # if poison_classwise:
            ...
            #         self.poison_samples_idx.append(i)
        # else:
        targets = list(range(0, len(self.filename)))
        self.poison_samples_idx = sorted(np.random.choice(targets, int(len(targets) * poison_rate), replace=False).tolist())
        ...
        for idx in self.poison_samples_idx:
            ...
            if perturb_type == 'samplewise':
                # Sample Wise poison
                noise = perturb_tensor[idx]
                noise = patch_noise_extend_to_img(noise, [70, 70, 3], patch_location=self.patch_location)
            elif perturb_type == 'classwise':
                # Class Wise Poison
                noise = perturb_tensor[self.attr[idx, self.attribute_number]]
                noise = patch_noise_extend_to_img(noise, [70, 70, 3], patch_location=self.patch_location)
              ...
              # Class Wise Poison
                ...
                noise = patch_noise_extend_to_img(noise, [70, 70, 3], patch_location=self.patch_location)
            if add_uniform_noise:
                noise += np.random.uniform(0, 8, (70, 70, 3))
        ...
        print('add_uniform_noise: ', add_uniform_noise)
        print(self.perturb_tensor.shape)
        print('Poison samples: %d/%d' % (len(self.poison_samples), len(self.filename)))
...
class UTKFaceDataset(ImageFolder):
    def __init__(self, root, split = 'train', transform = None, target_transform=None, seed = 0, partial_rate=1.0):
        super().__init__(root=os.path.join(root, split), transform=transform, target_transform=target_transform)        
        self.transform = transform
        self.seed = seed
        random.seed(seed)
        torch.manual_seed(seed)
        def shuffle_with_seed_and_indices(lst, seed=42):
            indexed_lst = list(enumerate(lst))
            random.seed(seed)
            random.shuffle(indexed_lst)
            shuffled_indices, shuffled_lst = zip(*indexed_lst)
            return list(shuffled_lst), list(shuffled_indices)
        self.samples, shuffled_indices = shuffle_with_seed_and_indices(self.samples)
        print (shuffled_indices[:10])
        print (self.samples[0][0])
        ...
        for index in range(len(self.samples)):
            ...
            image = transforms.Resize((140, 140))(Image.open(path))
            age, gender, race = path.split("/")[-1].split(".")[0].split("_")[:-1]
            age = map_age(int(age))
            age = torch.tensor(age)
            gender = torch.tensor(int(gender))
            race = torch.tensor(int(race))
            self.image[index] = image
            self.age[index] = age
            self.gender[index] = gender
    ...
    def __len__(self):
        return int(len(self.samples) * self.partial_rate)
    ...
    def __getitem__(self,index):
        ...
        if not self.return_index:
            ...
            return image, [age, race, gender], torch.tensor(index)

class CIFAR10_Tensor(datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        super(CIFAR10_Tensor, self).__init__(root=root, train=train, download=download, transform=transform, target_transform=target_transform)
        self.w_transform = True
...
class PoisonUTKFaceDataset(ImageFolder):
    def __init__(self, root, split = 'train', transform=None, target_transform=None,
                 poison_rate=1.0, perturb_tensor_filepath=None,
                 seed=0, perturb_type='classwise', patch_location='center', img_denoise=False,
                 add_uniform_noise=False, poison_classwise=False, poison_classwise_idx=None, partial_rate=1.0):
        super().__init__(root=os.path.join(root, split), transform=transform, target_transform=target_transform)  
        self.partial_rate = partial_rate
        self.transform = transform
        self.seed = seed
        ...
        # Check Shape
        target_dim = self.perturb_tensor.shape[0] if len(self.perturb_tensor.shape) == 4 else self.perturb_tensor.shape[1]
        if perturb_type == 'samplewise' and target_dim != len(self.samples):
            raise('Poison Perturb Tensor size not match for samplewise')
        elif perturb_type == 'classwise' and (not (target_dim in [9,2,5])):
            raise('Poison Perturb Tensor size not match for classwise')
        ...
        # if poison_classwise:
            ...
            #         self.poison_samples_idx.append(i)
        # else:
        targets = list(range(0, len(self.samples)))
        self.poison_samples_idx = sorted(np.random.choice(targets, int(len(targets) * poison_rate), replace=False).tolist())
        # self.poison_samples_idx = list(range(int(len(targets) * poison_rate)))
        ...
        for idx in self.poison_samples_idx:
            ...
            if perturb_type == 'samplewise':
                # Sample Wise poison
                noise = perturb_tensor[idx]
                noise = patch_noise_extend_to_img(noise, [140, 140, 3], patch_location=self.patch_location)
            elif perturb_type == 'classwise':
                # Class Wise Poison
                noise = perturb_tensor[self.targets[idx]]
                noise = patch_noise_extend_to_img(noise, [140, 140, 3], patch_location=self.patch_location)
              ...
              # Class Wise Poison
                num_attr = int(perturb_type.split('_')[-1])
                binary_list = [self.age[idx], self.race[idx], self.gender[idx]][:num_attr]
                cumulative_list = [0, 9, 14, 16]
                noise = torch.zeros(perturb_tensor.shape[1], perturb_tensor.shape[2], perturb_tensor.shape[3])
                for i, val in enumerate(binary_list):
                    noise += perturb_tensor[cumulative_list[i] + val, :, :, :]
                noise = noise / num_attr
                noise = patch_noise_extend_to_img(noise, [140, 140, 3], patch_location=self.patch_location)
            elif 'classwise_patch' in perturb_type:
              # Class Wise Poison
                ...
                for i in range(N):
                    for j in range(N):
                        ...
                        if iidx >= num_attr:  # 如果超出 binary_list 的长度，跳过
                            break
                        patch_perturbation = perturb_tensor[cumulative_list[iidx] + binary_list[iidx], ::N, ::N, :]
                        noise[i * patch_size:(i + 1) * patch_size, j * patch_size:(j + 1) * patch_size, :] += patch_perturbation
                noise = patch_noise_extend_to_img(noise, [140, 140, 3], patch_location=self.patch_location)
            if add_uniform_noise:
                noise += np.random.uniform(0, 8, (140, 140, 3))
        ...
        print('add_uniform_noise: ', add_uniform_noise)
        print(self.perturb_tensor.shape)
        print('Poison samples: %d/%d' % (len(self.poison_samples), len(self.samples)))
...
class FairFaceDataset(Dataset):
    def __init__(self, root, split = 'train', transform = None, seed = 0):
        ...
        for index in range(len(self.data)):
            ...
            age = self.map2label['age'][self.data[index][1]]
            gender = self.map2label['gender'][self.data[index][2]]
            race = self.map2label['race'][self.data[index][3]] 
            age = torch.tensor(age)
            gender = torch.tensor(gender)
            race = torch.tensor(race)
            img_path = os.path.join(self.path, img_name)
            image = transforms.Resize((140, 140))(Image.open(img_path))
            self.image[index] = image
            self.age[index] = age
            self.gender[index] = gender
            self.race[index] = race
...
class PoisonCIFAR10_CMTL(datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=False, poison_rate=1.0, perturb_tensor_filepath=None,
                 seed=0, perturb_type='classwise', patch_location='center', img_denoise=False,
                 add_uniform_noise=False, poison_classwise=False, poison_classwise_idx=None, poison_samples_idx = None):
        super(PoisonCIFAR10_CMTL, self).__init__(root=root, train=train, download=download, transform=transform, target_transform=target_transform)
        self.perturb_tensor = torch.load(perturb_tensor_filepath, map_location=device)
        print(self.perturb_tensor)
        if len(self.perturb_tensor.shape) == 4:
            self.perturb_tensor = self.perturb_tensor.mul(255).clamp_(-255, 255).permute(0, 2, 3, 1).to('cpu').numpy()
    ...
    #         corruption_type = random.randint(1, 15) - 1
    #         severity = random.randint(1, 5) - 1
    #         corrupted = corrupt(image, corruption_number=corruption_type, severity=severity+1)
    #         target_list.append(torch.tensor(severity + corruption_type * 5))
    #         corrupted = Image.fromarray(corrupted)
    #         if self.transform is not None:
    #             corrupted = self.transform(corrupted)
    #         img_list.append(corrupted)
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(repeat):
            ...
            if severity > 0:
                corrupted = corrupt(image, corruption_number=corruption_type, severity=5)
            else:
                corrupted = image
            target_list.append(torch.tensor(severity + corruption_type * 5))
            corrupted = Image.fromarray(corrupted)
...
class CIFAR10_CMTL(datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        super(CIFAR10_CMTL, self).__init__(root=root, train=train, download=download, transform=transform, target_transform=target_transform)
